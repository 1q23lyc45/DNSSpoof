...
def arg_parser():
    ...
    parser.add_argument("-d", dest="domain",
                        help="Domain to spoof (default: all)")
    parser.add_argument("-i", dest="interface",
                        help="Interface to listen on (default: default)")
    parser.add_argument("-t", dest="target",
                        help="Target's IP. If not specified (default: all)")
    parser.add_argument("-r", dest="ip",
                        help="IP to redirect the target to (default: local IP)")
...
def main():
    ...
    # Creating the DNS spoofing process
    stop_event = multiprocessing.Event()
    dns = multiprocessing.Process(target=DNS_spoof, args=(interface, domain, target, ip, stop_event))
    dns.start()
    ...
    try:
        ...
        # Stop the processes (ARP and DNS spoofing)
        stop_event.set()
